...
export const romberg = {
	...
	integrateFresnel : function(tauFunc, a, b) { // console.log("EULER!");
		...
		for (let idx = 1; idx <= romberg.NMAX; idx++) {
			...
			for (let jdx = 1; jdx <= idx; jdx++) {
				...
				// returning the value if either tolerance is met
				if (EaC < romberg.abs && EaS < romberg.abs) { console.debug("romberg at " + idx); return integ_val; }
				if (ErC < romberg.rel && ErS < romberg.rel) { console.debug("romberg at " + idx); return integ_val; }
			}
		}
		// returning the last calculated value of integral whether tolerance is met or not
