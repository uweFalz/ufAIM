...
export function buildHorizontalSequenceAddRequest({ type, length, arcAuthority = null, signedValue = null, transitionType = null, w1 = null, w2 = null, availableTransitionTypes = [] } = {}) {
	...
	const args = { length: explicitLength, transitionType: family };
	...
	return Object.freeze({ ok: true, method: "transition", args: Object.freeze(args) });
}
...
export function makeAlignmentCreationController({ store, messaging, curvatureBand, cockpit, viewController } = {}) {
	...
	function renderAddFields() {
		...
		for (const selector of ["[data-add-arc-authority]", "[data-add-signed-value]"]) { const field = root.querySelector(selector); if (field) field.hidden = type !== "arc"; }
	...
	async function create({ name } = {}) {
		const result = await createAlignmentFromExplicitName(editor, { name }); feedback(result, "alignment_creation.created"); return result?.changed ? sync(result) : result;
	}
	async function add(request) {
		...
		else if (qualified.method === "transition") result = await editor.addTransitionToActiveAlignment(qualified.args);
