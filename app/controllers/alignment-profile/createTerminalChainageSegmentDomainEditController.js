...
export class TerminalChainageSegmentDomainEditError extends Error {
	constructor(code, message, { cause } = {}) {
		super(message, cause === undefined ? undefined : { cause });
		this.name = "TerminalChainageSegmentDomainEditError";
		this.code = code;
		if (cause !== undefined) this.cause = cause;
	}
}

const fail = (code, message) => new TerminalChainageSegmentDomainEditError(code, message);

function requireId(value, label) {
	const id = typeof value === "string" ? value.trim() : "";
	if (!id) throw fail("INVALID_TERMINAL_CHAINAGE_DOMAIN_EDIT", `${label} must be a non-empty string`);
	return id;
}

function finite(value, label) {
	if (typeof value === "string" && value.trim() === "") {
		throw fail("INVALID_TERMINAL_CHAINAGE_DOMAIN_EDIT", `${label} must be finite`);
	}
	const number = Number(value);
	if (!Number.isFinite(number)) throw fail("INVALID_TERMINAL_CHAINAGE_DOMAIN_EDIT", `${label} must be finite`);
	return number;
}
...
function requireTarget(profileState, alignmentId, mappingId, segmentId) {
	if (!profileState || typeof profileState !== "object" || Array.isArray(profileState) ||
		...
		throw fail("INVALID_TERMINAL_CHAINAGE_DOMAIN_EDIT", "canonical profileState is required");
	}
...
function rebuildMapping(mapping, target, endS) {
	...
	for (const segment of mapping.segments) {
		rebuilt = appendChainageSegment(rebuilt, Object.is(segment, target) ? { ...segment, endS } : segment);
	}
	return rebuilt;
}
...
export function createTerminalChainageSegmentDomainEditController({
	...
	if (typeof alignmentProfileApplicationService?.saveProfileState !== "function" ||
		...
		throw fail("INVALID_SERVICE", "terminal chainage domain editing requires profile save and projection services");
	}
	return Object.freeze({
		async update({ alignmentId, revision, s, profileState, mappingId, segmentId, endS } = {}) {
			const normalizedAlignmentId = requireId(alignmentId, "alignmentId");
			if (revision === undefined) throw fail("INVALID_TERMINAL_CHAINAGE_DOMAIN_EDIT", "revision must be explicit");
			...
			if (Object.is(numericEndS, persisted.segment.endS)) {
				throw fail("TERMINAL_CHAINAGE_DOMAIN_NO_CHANGE", "endS is unchanged");
			}
			if (numericEndS <= persisted.segment.startS) {
				throw fail("INVALID_TERMINAL_CHAINAGE_DOMAIN_EDIT", "endS must be greater than the terminal segment startS");
			}
			...
			let mapping;
			try {
				mapping = rebuildMapping(persisted.mapping, persisted.segment, numericEndS);
			} catch (cause) {
				throw new TerminalChainageSegmentDomainEditError(
					"INVALID_TERMINAL_CHAINAGE_DOMAIN_EDIT", "terminal chainage segment domain is invalid", { cause }
				);
			}
			...
			try {
				...
				throw new TerminalChainageSegmentDomainEditError(
					"TERMINAL_CHAINAGE_DOMAIN_SAVE_FAILED", "terminal chainage segment domain was not saved", { cause }
				);
			}
			if (!snapshotMatches(snapshot, nextProfileState)) {
				throw fail("PROFILE_READBACK_MISMATCH", "saved terminal chainage domain does not match repository readback");
			}
...
export default createTerminalChainageSegmentDomainEditController;
