...
export class TerminalConstantCantDomainEditError extends Error {
	constructor(code, message, { cause } = {}) {
		super(message, cause === undefined ? undefined : { cause });
		this.name = "TerminalConstantCantDomainEditError";
		this.code = code;
		if (cause !== undefined) this.cause = cause;
	}
}

const fail = (code, message) => new TerminalConstantCantDomainEditError(code, message);

function requireId(value, label) {
	const id = typeof value === "string" ? value.trim() : "";
	if (!id) throw fail("INVALID_TERMINAL_CONSTANT_CANT_DOMAIN_EDIT", `${label} must be a non-empty string`);
	return id;
}

function finite(value, label) {
	if (typeof value === "string" && value.trim() === "") throw fail("INVALID_TERMINAL_CONSTANT_CANT_DOMAIN_EDIT", `${label} must be finite`);
	const number = Number(value);
	if (!Number.isFinite(number)) throw fail("INVALID_TERMINAL_CONSTANT_CANT_DOMAIN_EDIT", `${label} must be finite`);
	return number;
}
...
export function createTerminalConstantCantDomainEditController({ alignmentProfileApplicationService, projectionController } = {}) {
	if (typeof alignmentProfileApplicationService?.saveProfileState !== "function" || typeof projectionController?.projectAt !== "function") throw fail("INVALID_SERVICE", "terminal constant Cant domain editing requires profile save and projection services");
	return Object.freeze({
		async update({ alignmentId, revision, s, profileState, elementId, endS } = {}) {
			...
			if (revision === undefined) throw fail("INVALID_TERMINAL_CONSTANT_CANT_DOMAIN_EDIT", "revision must be explicit");
			...
			if (Object.is(numericEndS, persisted.target.endS)) throw fail("TERMINAL_CONSTANT_CANT_DOMAIN_NO_CHANGE", "endS is unchanged");
			if (numericEndS <= persisted.target.startS) throw fail("INVALID_TERMINAL_CONSTANT_CANT_DOMAIN", "endS must be greater than the persisted startS");
			...
			if (![evaluation.start, evaluation.interior, evaluation.end].every((entry) => entry.elementId === target.id && Object.is(entry.crossLevel, target.startCrossLevel) && Object.is(entry.twist, 0))) throw fail("CANT_STATE_VALIDATION_FAILED", "canonical Cant evaluation does not match the edited constant domain");
			...
			catch (cause) { throw new TerminalConstantCantDomainEditError("TERMINAL_CONSTANT_CANT_DOMAIN_SAVE_FAILED", "terminal constant Cant domain was not saved", { cause }); }
			if (!snapshotMatches(snapshot, nextProfileState)) throw fail("PROFILE_READBACK_MISMATCH", "saved terminal constant Cant domain does not match repository readback");
...
export default createTerminalConstantCantDomainEditController;
