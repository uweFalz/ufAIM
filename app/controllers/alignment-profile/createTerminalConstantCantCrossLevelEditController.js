...
export class TerminalConstantCantCrossLevelEditError extends Error {
	constructor(code, message, { cause } = {}) {
		super(message, cause === undefined ? undefined : { cause });
		this.name = "TerminalConstantCantCrossLevelEditError";
		this.code = code;
		if (cause !== undefined) this.cause = cause;
	}
}

const fail = (code, message) => new TerminalConstantCantCrossLevelEditError(code, message);

function requireId(value, label) {
	const id = typeof value === "string" ? value.trim() : "";
	if (!id) throw fail("INVALID_TERMINAL_CONSTANT_CANT_EDIT", `${label} must be a non-empty string`);
	return id;
}

function finite(value, label) {
	if (typeof value === "string" && value.trim() === "") throw fail("INVALID_TERMINAL_CONSTANT_CANT_EDIT", `${label} must be finite`);
	const number = Number(value);
	if (!Number.isFinite(number)) throw fail("INVALID_TERMINAL_CONSTANT_CANT_EDIT", `${label} must be finite`);
	return number;
}
...
function requireTarget(profileState, alignmentId, elementId) {
	...
	if (target.type !== "constant-cross-level") throw fail("CANT_ELEMENT_NOT_CONSTANT", "the selected Cant element is not constant-cross-level");
	if (!Object.is(target, profileState.cant.elements.at(-1))) throw fail("CANT_ELEMENT_NOT_TERMINAL", "only the terminal constant Cant element may be edited");
	return { profileState, cant: profileState.cant, target };
}

function rebuildCantState(cant, target, crossLevel) {
	...
	for (const element of cant.elements) rebuilt = appendCantElement(rebuilt, Object.is(element, target) ? { ...element, startCrossLevel: crossLevel } : element);
	return rebuilt;
}
...
export function createTerminalConstantCantCrossLevelEditController({ alignmentProfileApplicationService, projectionController } = {}) {
	if (typeof alignmentProfileApplicationService?.saveProfileState !== "function" || typeof projectionController?.projectAt !== "function") throw fail("INVALID_SERVICE", "terminal constant Cant editing requires profile save and projection services");
	return Object.freeze({
		async update({ alignmentId, revision, s, profileState, elementId, crossLevel } = {}) {
			const normalizedAlignmentId = requireId(alignmentId, "alignmentId");
			if (revision === undefined) throw fail("INVALID_TERMINAL_CONSTANT_CANT_EDIT", "revision must be explicit");
			const cursorS = finite(s, "s"), normalizedElementId = requireId(elementId, "elementId"), numericCrossLevel = finite(crossLevel, "crossLevel");
			const persisted = requireTarget(profileState, normalizedAlignmentId, normalizedElementId);
			if (Object.is(numericCrossLevel, persisted.target.startCrossLevel)) throw fail("TERMINAL_CONSTANT_CANT_NO_CHANGE", "crossLevel is unchanged");
			...
			const cant = rebuildCantState(persisted.cant, persisted.target, numericCrossLevel);
			...
			if (![evaluation.start, evaluation.interior, evaluation.end].every((entry) => entry.elementId === target.id && Object.is(entry.crossLevel, numericCrossLevel) && Object.is(entry.twist, 0))) throw fail("CANT_STATE_VALIDATION_FAILED", "canonical Cant evaluation does not match the edited constant element");
			...
			catch (cause) { throw new TerminalConstantCantCrossLevelEditError("TERMINAL_CONSTANT_CANT_SAVE_FAILED", "terminal constant Cant value was not saved", { cause }); }
			if (!snapshotMatches(snapshot, nextProfileState)) throw fail("PROFILE_READBACK_MISMATCH", "saved terminal constant Cant value does not match repository readback");
...
export default createTerminalConstantCantCrossLevelEditController;
