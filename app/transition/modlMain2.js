...
// =============================
// 🧭 Transition Profile Manager
// =============================

const transitionProfileManager = {
	useAdvancedOptions: false,

	standardProfiles: ["clothoid", "bloss"],

	getAvailableProfiles() {
		return this.useAdvancedOptions
		? Object.keys(transitionTypeDefs)
		: this.standardProfiles;
	},

	getProfileInfo(name) {
		const def = transitionTypeDefs[name];
		return def
		? {
			name: def.name,
			origin: def.origin,
			class: def.class,
			grade: def.sourceGrade,
			composition: def.composition,
			domain: def.segments?.[0]?.domain || [0, 1]
		}
		: null;
	},

	previewToBoard(name, board) {
		const def = transitionTypeDefs[name];
		if (!def) return;
		const κ = buildUnifiedKappa(def);
		plotToJSXGraph(κ, board, name);
	},

	toggleAdvanced(state) {
		this.useAdvancedOptions = state;
		updateTransitionSelectorUI();
	}
};

// =============================
// 🎛️ UI: Transition Selector + Preview
// =============================

function updateTransitionSelectorUI() {
	const container = document.getElementById("transition-ui");
	if (!container) return;
	...
	toggle.id = "expert-mode";
	toggle.checked = transitionProfileManager.useAdvancedOptions;
	toggle.onchange = () => transitionProfileManager.toggleAdvanced(toggle.checked);

	const toggleLabel = document.createElement("label");
	toggleLabel.htmlFor = "expert-mode";
	toggleLabel.textContent = " Experte";

	const select = document.createElement("select");
	select.id = "transition-select";

	transitionProfileManager.getAvailableProfiles().forEach((name) => {
	...
	previewBtn.onclick = () => {
		...
		const board = window.jsxBoard;
		if (board) transitionProfileManager.previewToBoard(name, board);
		const info = transitionProfileManager.getProfileInfo(name);
		if (info) {
			document.getElementById("profile-info").textContent =
			`Typ: ${info.name} | Klasse: ${info.class} | Ursprung: ${info.origin} | Ableitung: ${info.grade}`;
		}
	};

	const infoBox = document.createElement("div");
	infoBox.id = "profile-info";
	infoBox.style.cssText = "margin-top:4px;color:#444;font-size:0.85em";
...
console.log("🧭 Transition profile manager + UI ready.");
