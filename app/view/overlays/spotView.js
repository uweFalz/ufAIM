...
export function renderSpotHtml({ spotState, storeState, query = "", capabilities = {}, interaction = {} }) {
	...
	const pendingImportCount = spotState?.importActivity?.state === "completed" ? Math.max(0, Number(spotState?.importActivity?.objectCount ?? 0)) : 0;
	const hasPendingImport = rows.length === 0 && pendingImportCount > 0;
	const emptyTitle = hasPendingImport
		? t(pendingImportCount === 1 ? "spot_workspace_pending_import_title_one" : "spot_workspace_pending_import_title", { count: pendingImportCount })
		: t("spot_workspace_empty_title");
	const emptyText = hasPendingImport ? t("spot_workspace_pending_import_text") : t("spot_workspace_empty_text");
	...
	${rows.length === 0 ? `<div class="spotWorkspace__empty" ${hasPendingImport ? `data-spot-pending-import="${escapeHtml(pendingImportCount)}"` : ""}><h3>${escapeHtml(emptyTitle)}</h3><p>${escapeHtml(emptyText)}</p><div>${hasPendingImport && capabilities.reviewImport ? `<button data-spot-review-import>${escapeHtml(t("spot_workspace_review_import"))}</button>` : ""}${capabilities.create ? `<button data-spot-create>${escapeHtml(t("spot_workspace_create"))}</button>` : ""}${capabilities.import ? `<button data-spot-import>${escapeHtml(t("spot_workspace_import"))}</button>` : ""}</div></div>` : visible.length ? `<div class="spotWorkspace__navigator" data-spot-route-navigator>${groupedHtml}${reviewHtml}</div>` : `<div class="spotWorkspace__empty"><h3>${escapeHtml(t("spot_workspace_no_match_title"))}</h3><p>${escapeHtml(t("spot_workspace_no_match_text"))}</p></div>`}
...
export function makeSpotView({ rootEl } = {}) {
	...
	const capabilities = () => ({ rename: typeof actions.onRename === "function", remove: typeof actions.onRemove === "function", create: typeof actions.onCreate === "function", import: typeof actions.onImport === "function", reviewImport: typeof actions.onReviewImport === "function", edit: typeof actions.onEdit === "function" });
	...
	rootEl.addEventListener("click", async (event) => {
		...
		if (event.target.closest("[data-spot-review-import]")) return actions.onReviewImport?.();
