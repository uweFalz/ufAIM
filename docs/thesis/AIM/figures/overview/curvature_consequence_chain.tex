% figures/overview/curvature_consequence_chain.tex
\begin{tikzpicture}[
  x=1cm,
  y=1cm,
  >=Stealth,
  ...
  \node[label, anchor=north] at ([yshift=-3pt]kappa.north) {Curvature band\\$\kappa(s)$};
  ...
  \node[note, anchor=south] at ([yshift=3pt]kappa.south) {intrinsic construction};

  \node[stage, right=0.55cm of kappa] (plan) {};
  \node[label, anchor=north] at ([yshift=-3pt]plan.north) {Plan geometry\\$\gamma(s)$};
  \draw[very thick, black!75]
  ...
  \node[note, anchor=south] at ([yshift=3pt]plan.south) {metric interpretation};

  \node[stage, right=0.55cm of plan] (context) {};
  \node[label, anchor=north] at ([yshift=-3pt]context.north) {Engineering\\context};
  \draw[black!25] ([xshift=-1.10cm,yshift=-0.45cm]context.center) grid[xstep=.45cm,ystep=.38cm]
  ...
  \node[note, anchor=south] at ([yshift=3pt]context.south) {reality, rules, observations};

  \node[stage, right=0.55cm of context] (use) {};
  \node[label, anchor=north] at ([yshift=-3pt]use.north) {Purposeful\\accounts};
  \node[draw=black!45, rounded corners=1pt, font=\scriptsize, minimum width=.72cm]
    at ([xshift=-.78cm,yshift=.25cm]use.center) {GIS};
  \node[draw=black!45, rounded corners=1pt, font=\scriptsize, minimum width=.72cm]
    at ([yshift=.25cm]use.center) {CAD};
  \node[draw=black!45, rounded corners=1pt, font=\scriptsize, minimum width=.72cm]
    at ([xshift=.78cm,yshift=.25cm]use.center) {BIM};
  \node[note] at ([yshift=-.32cm]use.center) {evaluation\;\textbullet\; consequence};
  \node[note, anchor=south] at ([yshift=3pt]use.south) {evidence for human decision};
