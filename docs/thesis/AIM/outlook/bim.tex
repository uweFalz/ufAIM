...
→ reviewable import candidates
→ explicit admission or rejection
→ admitted engineering object and qualified state
...
\item evidence review and admission responsibility,
\item computational modelling,
\item engineering reasoning.
\end{itemize}

A successful parse is therefore not an admission decision. Imported source
records may remain partial, ambiguous, or unresolved, and those limitations
must remain attached to their provenance. Only an explicit admission step can
make a durable engineering object available to downstream services. The
current Reference Implementation is one demonstration of this separation; it
maps such an object to a SpotObject in its loaded SPOT universe, but neither
implementation term defines Kernel meaning or authority. Durable
distinguishability remains governed by the active KC-ID-001 Engineering Object
Identity record.
...
Existing BIM data is examined via:
\begin{itemize}
\item IFC parsing,
...
Within AIM, admitted information may be:
\begin{itemize}
\item normalized,
\item reconstructed,
...
Information that has not been admitted remains import evidence. Missing object
relations, coordinate meaning, stationing interpretation, or source authority
must not be inferred from filenames, import order, or representational
similarity. Several synchronized workspace views may present an admitted
Alignment, but they remain representations of the same object and revision.
