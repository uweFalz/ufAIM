...
\section{Skalierung}
\begin{definition}[Skaliertes Übergangsgesetz]
\label{def:scaled-transition-law}
Seien \(L>0\), \(\kappa_0,\kappa_1\in\R\) und \(\widehat{\kappa}:[0,1]\to\R\)
ein normiertes Übergangsgesetz mit \(\widehat{\kappa}(0)=0\) und
\(\widehat{\kappa}(1)=1\). Das skalierte Übergangsgesetz ist
\[
\kappa(s)=\kappa_0+(\kappa_1-\kappa_0)\,
\widehat{\kappa}\!\left(\frac{s}{L}\right),
\qquad s\in[0,L].
\]
Es erfüllt nach Konstruktion \(\kappa(0)=\kappa_0\) und
\(\kappa(L)=\kappa_1\).
\end{definition}
