...
\begin{proposition}[Derivative structure]
\label{prop:parametric-derivative-structure}
For a shape parameter \(p\in\mathbf{T}\), with \(L\), \(\kappa_0\), and
\(\kappa_1\) fixed,
\[
...
\]
For the remaining parameters the derivatives carry additional terms:
\[
...
\]
where \(\partial_1\) denotes the derivative with respect to the first
argument.
\end{proposition}

\begin{proof}
Differentiate the parametric curvature model term by term; the
\(L\)-derivative follows from the chain rule applied to \(s/L\).
\end{proof}

Thus, all derivatives are computed analytically from prototype
functions, provided the parameter type is respected.
