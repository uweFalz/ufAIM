% optimization/alignment/20_discrete_formulation.tex
...
\subsubsection{Model-reference-point candidate objective}

For an explicitly identified reference-point model \(M\), finite differences
may approximate
\[
\|\gamma_{P_M}''(s_i)\|^2
\]
by second-order differences of \(\gamma_{P_M}\):
...
This candidate objective inherits the model, frame, discretization and
applicability limits stated in the Schwerpunkt example. It is not a physical
centre-of-mass, comfort or safety objective unless an identified model and
responsible source establish that interpretation.
...
The four terms have different meanings and may have different units.
Reference scales, weights \(\alpha,\beta,\gamma,\delta\), their source and
purpose must be declared before this expression defines an optimization
problem. A solver result remains a candidate.
