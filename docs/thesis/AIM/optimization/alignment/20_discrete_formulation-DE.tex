...
\]
\subsubsection{Mögliches Ziel eines Modellbezugspunkts}
Für ein ausdrücklich identifiziertes Bezugspunktmodell \(M\) kann die Größe
\[
\|\gamma_{P_M}''(s_i)\|^2
\]
durch finite Differenzen zweiter Ordnung approximiert werden:
\[
J_{P_M}(\mathbf x)=\sum_{i=1}^{N-1}
\|\gamma_{P_M,i+1}-2\gamma_{P_M,i}+\gamma_{P_M,i-1}\|^2.
\]
Dieses mögliche Ziel erbt Modell-, Frame-, Diskretisierungs- und
Anwendbarkeitsgrenzen des Schwerpunktbeispiels. Es ist kein Ziel für
physischen Massenmittelpunkt, Komfort oder Sicherheit, solange dies nicht ein
identifiziertes Modell und eine verantwortliche Quelle begründen.

\subsection{Kombiniertes diskretes Problem}
\begin{definition}[Diskrete Zielfunktion]
\[
J(\mathbf x)=\alpha J_{P_M}+\beta J_{\mathrm{dyn}}
+\gamma J_\kappa+\delta J_\phi.
\]
\end{definition}
Die vier Terme haben verschiedene Bedeutungen und möglicherweise verschiedene
Einheiten. Referenzskalen, Gewichte \(\alpha,\beta,\gamma,\delta\), ihre Quelle
und ihr Zweck müssen erklärt sein, bevor dieser Ausdruck ein
Optimierungsproblem definiert. Das Solver-Ergebnis bleibt ein Kandidat.
