...
test("workspace start opens the explicit-name form instead of attempting a nameless save", () => {
	const init = read("app/runtime/init/initFeatures.js");
	assert.match(init, /createAlignment: \(\) => ctx\.gndImportWorkbench\?\.openCreation\?\.\(\)/);
	assert.doesNotMatch(init, /createAlignment: \(\) => ctx\.gndImportWorkbench\?\.createAlignment\?\.\(\)/);
});
