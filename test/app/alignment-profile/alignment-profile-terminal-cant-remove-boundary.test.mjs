...
test("controller uses only canonical Cant reconstruction and evaluation", async () => {
	const source = await read("app/controllers/alignment-profile/createTerminalCantElementRemoveController.js");
	...
	assert.doesNotMatch(source, /crossLevel\s*[+*]|crossLevelRate\s*\*|Spot\.|Worker|Messaging|IndexedDB|localStorage|document|window|rail|datum|untertief/i);
});

test("view remains import-free and wiring owns canonical readback", async () => {
	...
	assert.doesNotMatch(view, /createCantConstructiveState|appendCantElement|evaluateCantAt|saveProfileState|repository|Spot\.|sendCmdAwait/);
	assert.match(wiring, /removeTerminalCantElement/);
	...
	assert.match(init, /createTerminalCantElementRemoveController/);
