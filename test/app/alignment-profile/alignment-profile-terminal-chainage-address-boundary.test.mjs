...
test("controller imports only canonical Chainage create append forward inverse APIs", async () => {
	const source = await read("app/controllers/alignment-profile/createTerminalChainageSegmentAddressEditController.js");
...
test("view remains import-free and wiring alone owns canonical readback", async () => {
	...
	assert.match(wiring, /updateTerminalChainageSegmentAddress/);
	...
	assert.match(init, /createTerminalChainageSegmentAddressEditController/);
