...
const projection = (revision = 1) => ({ status: "projected", alignmentId: "A1", revision,
	...
	vertical: { status: "not-covered" }, chainage: { status: "evaluated", candidates: [{ segmentId: "CHS2", address: 2975 }] }, cant: { status: "evaluated" } });

test("view submits only explicit mapping segment and address values", () => {
	const documentRef = new Document();
	const host = new Element(documentRef);
	const view = new AlignmentProfileSynchronizedView({ host });
	let submitted;
	view.setTerminalChainageAddressEditHandler((value) => { submitted = value; });
	view.render(projection());
	const details = host.find((node) => Object.hasOwn(node.dataset, "terminalChainageAddressEdit"));
	assert.ok(details);
	const values = { addressEditMappingId: "CH1", addressEditSegmentId: "CHS2", addressEditStartAddress: "3000" };
	for (const [name, value] of Object.entries(values)) {
		const input = details.find((node) => node.name === name);
		assert.equal(input.value, "");
		input.value = value;
	}
	details.find((node) => node.tag === "button" && node.textContent === "Apply terminal start address").onclick();
	assert.deepEqual(submitted, { mappingId: "CH1", segmentId: "CHS2", startAddress: "3000" });
});

test("form is absent when canonical chainage evidence is absent", () => {
	const documentRef = new Document();
	const host = new Element(documentRef);
	const view = new AlignmentProfileSynchronizedView({ host });
	view.setTerminalChainageAddressEditHandler(() => {});
	view.render({ ...projection(), chainage: { status: "absent", mappings: [] } });
	assert.equal(host.find((node) => Object.hasOwn(node.dataset, "terminalChainageAddressEdit")), null);
});

test("wiring uses canonical context and renders only matching SPOT readback", async () => {
	const documentRef = new Document();
	let revision = 1;
	...
	let received;
	class View { setTerminalChainageAddressEditHandler(handler) { this.handler = handler; } renderTerminalChainageAddressEditStatus(value) { this.status = value; } render(value) { this.value = value; } }
	const wiring = wireAlignmentProfileSynchronizedView({
		...
		terminalChainageSegmentAddressEditController: { async update(value) { received = value; revision = 2; profileState = { ...value.profileState, chainageMappings: [{ ...value.profileState.chainageMappings[0], edited: true }] }; return { snapshot: { revision }, profileState, projection: projection(revision) }; } },
		View,
	});
	const result = await wiring.updateTerminalChainageSegmentAddress({ mappingId: "CH1", segmentId: "CHS2", startAddress: "3000" });
