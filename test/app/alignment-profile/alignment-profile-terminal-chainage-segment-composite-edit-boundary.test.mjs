...
test("controller imports only canonical Chainage replay and mapping APIs", async () => {
	const source = await read("app/controllers/alignment-profile/createTerminalChainageSegmentCompositeEditController.js");
...
test("view stays import-free and wiring exclusively owns canonical readback", async () => {
	...
	assert.match(wiring, /updateTerminalChainageSegmentComposite/);
	...
	assert.match(init, /createTerminalChainageSegmentCompositeEditController/);
