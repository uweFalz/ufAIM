...
const IMPLEMENTATION_URL = new URL("symDiff.js", AST_ROOT);
const SYMDIFF_API = ["symDiff"];
...
test("Transition AST Core retains evaluator simplifier differentiation and barrel", async () => {
	const files = (await readdir(AST_ROOT)).filter((name) => name.endsWith(".js"));
	for (const expected of ["evalAst.js", "index.js", "simplify.js", "symDiff.js"]) {
		assert.equal(files.includes(expected), true, expected);
	}
});

test("canonical symDiff imports exactly six symbols from canonical simplify", async () => {
...
test("canonical symDiff has no forbidden dependency", async () => {
	const source = await readFile(IMPLEMENTATION_URL, "utf8");
...
test("AST barrel retains the accepted API plus symDiff", async () => {
...
test("Transition and Root barrels expose symDiff by identity", async () => {
	const canonical = await import("../../../src/aim-core/transition/ast/symDiff.js");
	const transition = await import("../../../src/aim-core/transition/index.js");
	const root = await import("../../../src/aim-core/index.js");
	assert.deepEqual(Object.keys(canonical), SYMDIFF_API);
	assert.strictEqual(transition.symDiff, canonical.symDiff);
	assert.strictEqual(root.symDiff, canonical.symDiff);
});
...
test("canonical symDiff never imports its legacy facade", async () => {
	const source = await readFile(IMPLEMENTATION_URL, "utf8");
	assert.equal(
		importSpecifiers(source).some((specifier) =>
			specifier.includes("domain/transition/registry/ast/symDiff")
		),
		false
	);
});
