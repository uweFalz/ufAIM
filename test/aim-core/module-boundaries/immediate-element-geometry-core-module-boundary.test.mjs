...
const CANONICAL_URL = new URL(
	"src/aim-core/geometry/ImmediateElement.js",
	REPOSITORY_ROOT
);
...
test("canonical ImmediateElement imports only canonical TransitionElement", async () => {
	const source = await readFile(CANONICAL_URL, "utf8");
	assert.deepEqual(importSpecifiers(source), ["./TransitionElement.js"]);
	assert.match(
		source,
...
test("canonical ImmediateElement has no forbidden dependency", async () => {
	const source = await readFile(CANONICAL_URL, "utf8");
...
test("canonical ImmediateElement exposes exactly one class", async () => {
	const canonical = await import(
		"../../../src/aim-core/geometry/ImmediateElement.js"
	);
	assert.deepEqual(Object.keys(canonical), ["ImmediateElement"]);
	assert.equal(typeof canonical.ImmediateElement, "function");
});

test("legacy ImmediateElement is an exact logic-free canonical facade", async () => {
	assert.equal(
		await readFile(LEGACY_URL, "utf8"),
		'export { ImmediateElement } from "../../../aim-core/geometry/ImmediateElement.js";\n'
	);
});

test("Geometry and Root expose ImmediateElement by identity", async () => {
	const canonical = await import(
		"../../../src/aim-core/geometry/ImmediateElement.js"
	);
...
test("canonical ImmediateElement never imports its legacy facade", async () => {
	const source = await readFile(CANONICAL_URL, "utf8");
	assert.doesNotMatch(source, /src\/domain|domain\/alignment|src\/lib/);
	assert.deepEqual(importSpecifiers(source), ["./TransitionElement.js"]);
});

test("legacy construction receives canonical ImmediateElement authority", async () => {
	...
	assert.ok(new legacy.ImmediateElement() instanceof canonical.ImmediateElement);
});
