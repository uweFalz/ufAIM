...
//        [--iterations 1000] [--hessian auto|bfgs|gauss-newton] [--restoration on-verdict|eager|off] [--lengthPrior sigma] [--correctionClosure 0.1] [--kinkStation held|free] [--qpWarmStart false] [--undeterminedVerdict false] [--corridor true] [--correctionRows equalities|all|active|violated] [--correctionMetric identity|hessian] [--pinnedWeight 1e-6] [--json out.json]
//
...
const pinnedWeight = args.pinnedWeight === undefined ? undefined : Number(args.pinnedWeight);
...
for (const { file, n } of trusted.slice(from, to)) {
	...
	for (const objective of objectives) {
		const t0 = Date.now();
		let run;
		const solver = { hessian, restoration, structuredStart, hybridSwitch, lengthPrior, acceptance, filterSwitching, filterCeiling, correctionClosure, qpWarmStart, undeterminedVerdict, corridor, correctionRows, correctionMetric, pinnedWeight };
